...
function buildFacets(elements, groups, divisor) {
  const facets = [];
  const held = new Map();
  ...
  for (const element of elements) {
    ...
    held.set(element, { ...(held.get(element) || {}), group });
  }
  if (used.size) {
    const named = new Map(groups.map(({ ng, title }) => [ng, title]));
    facets.push({
      id: "group",
      title: "Group",
      values: [...used.keys()]
        .sort((left, right) => left - right)
        .map((ng) => ({ id: ng, ...(named.get(ng) ? { title: named.get(ng) } : {}) })),
    });
  }

  // The geometric entity a member was generated along. A quad states no such
  // thing, so this covers line elements and says so by simply not naming them.
  const axes = new Map();
  for (const element of elements) {
    const axis = element.referenceAxis;
    if (!Number.isFinite(axis) || axis <= 0) continue;
    axes.set(axis, (axes.get(axis) || 0) + 1);
    held.set(element, { ...(held.get(element) || {}), referenceAxis: axis });
  }
  if (axes.size) {
    facets.push({
      id: "referenceAxis",
      title: "Reference axis",
      values: [...axes.keys()].sort((left, right) => left - right).map((id) => ({ id })),
    });
  }

  for (const [element, values] of held) element.facetValues = values;
  // `referenceAxis` is how a member said which axis it came from, and it is not
  // part of the contract - the facet is.
...
async function buildGeometry(database, gravityAxis, groupDivisor = 0) {
  ...
  const facets = buildFacets(elements, readGroups(groupRead), groupDivisor);

  return { nodes, elements, supports, sections, ...(facets.length ? { facets } : {}) };
}
