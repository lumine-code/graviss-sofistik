...
function readQuads(read, numbers, nodesById) {
  ...
  for (let index = 0; index < read.count; index += 1) {
    ...
    // The element is eccentric by half of what it measures, and a tapering one
    // by half of its thickest corner: that is the face the nodes sit on.
    const eccentricity = Array.isArray(thickness) ? Math.max(...thickness) : thickness || 0;
