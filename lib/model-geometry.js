...
// they span, and the cross-section they carry. Neither stores a local frame the
// way a beam does - its T is the axis alone, which is the run between its nodes
// and nothing the viewer does not already have - because an axial member has no
// bending for a section orientation to matter to. So none is stated, and the
// viewer picks the roll rather than being handed an invented one.
function readAxialElements(read, numbers, kind) {
  const elements = [];
  for (let index = 0; index < read.count; index += 1) {
    const nr = read.columns.nr[index];
    const start = read.columns.node[index * 2];
...
function platesShape(read) {
  ...
  const generated = [];
  for (let index = 0; index < read.count; index += 1) {
    const thickness = read.columns.t[index];
    const from = [read.columns.ya[index], read.columns.za[index]];
    const to = [read.columns.ye[index], read.columns.ze[index]];
    if (!finitePositive(thickness)) continue;
    if (![...from, ...to].every(Number.isFinite)) continue;
    if (from[0] === to[0] && from[1] === to[1]) continue;
    const plate = { from, to, thickness };
    if ((read.columns.idp[index] & PAN_GENERATED) !== 0) generated.push(plate);
...
async function buildGeometry(database) {
  ...
  const elements = [
    ...
    ...readAxialElements(trussRead, numbers, "truss"),
    ...readAxialElements(cableRead, numbers, "cable"),
